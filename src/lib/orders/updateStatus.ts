...
type AdminClient = { supabase: ReturnType<typeof createAdminClient> };

// SHIPPED yalnızca tracking endpoint'inden set edilir; buradan seçilemez.
export const VALID_ORDER_STATUSES = ["PENDING", "PREPARING", "DELIVERED", "CANCELLED", "CANCEL_REQUESTED"];

/**
 * Tek bir siparişin durumunu günceller + yan etkileri uygular
 * (DELIVERED→deliveredAt, PREPARING→R2 foto sil, WhatsApp bildirim).
 * Hem tekli route hem toplu güncelleme bu fonksiyonu kullanır (DRY).
 */
export async function updateOrderStatus(admin: AdminClient, id: string, status: string) {
  if (!VALID_ORDER_STATUSES.includes(status)) {
    return { ok: false as const, error: "Invalid status", code: 400 };
  }
  ...
  if (!current) return { ok: false as const, error: "Not found", code: 404 };
  ...
  if (error) return { ok: false as const, error: error.message, code: 500 };
