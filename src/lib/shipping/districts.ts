// src/lib/shipping/districts.ts — 81 il ve 973 ilçe. Saf veri + saf eşleştirme, DB/ağ bağımlılığı YOK.
//
// Neden gerekli: ilçe serbest metin girildiğinde aynı yer "Altındağ"/"altındağ"/"ALTINDAĞ"
// diye üç ayrı değere dağılıyor. Taşıyıcı gönderi oluştururken ilçe adını eşleştiriyor
// (Aras `SetOrder` → `ReceiverTownName`), eşleşmeyen ad gönderiyi hataya düşürüyor.
//
// İlçenin plaka gibi resmî evrensel bir kodu YOKTUR — taşıyıcılar kendi iç kodlarını
// verir. Bu yüzden burada saklanan şey standart ADdır, kod değil.
//
...
/** İki haneli plaka koduna normalize eder; geçersizse null. */
function normalizeCityCode(code: string | null | undefined): string | null {
  if (!code) return null;
...
export function districtsOf(cityCode: string | null | undefined): string[] {
  ...
  return code ? DISTRICTS[code] : [];
}

/**
 * Serbest metin ilçe adını o ildeki resmî yazıma eşler; eşleşme yoksa null.
 * Eski serbest metin kayıtları ("altındağ") seçiciye taşımak için kullanılır.
 */
export function normalizeDistrict(
  ...
  const list = districtsOf(cityCode);
  if (list.length === 0) return null;
  const k = key(name);
  return list.find(d => key(d) === k) ?? null;
}

/** CustomSelect için hazır seçenek listesi (`cities.ts`'teki CITY_OPTIONS deseni). */
export function districtOptions(cityCode: string | null | undefined) {
  return districtsOf(cityCode).map(d => ({ value: d, label: d }));
}
