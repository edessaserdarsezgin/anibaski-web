...
import { type DocSectionKey } from "@/lib/documents";
import { getMakbuzConfig } from "@/lib/documents.server";
...
export default async function MakbuzPage({ params }: Props) {
  ...
  const qrDataUrl = config.showQr
    ? await QRCode.toDataURL(`${base}/siparisler/${order.id}?from=admin`, { margin: 1, width: 220 })
    : null;
  ...
  const sectionNodes: Record<DocSectionKey, React.ReactNode> = {
    meta: (
      <div key="meta" className="grid grid-cols-3 gap-3 text-sm mb-4">
        <div><p className="text-black/50 text-xs">Sipariş Tarihi</p><p className="font-semibold">{new Date(order.createdAt).toLocaleDateString("tr-TR")}</p></div>
        <div><p className="text-black/50 text-xs">Durum</p><p className="font-semibold">{STATUS_LABEL[order.status] ?? order.status}</p></div>
        <div><p className="text-black/50 text-xs">Tür</p><p className="font-semibold">{order.type === "reprint" ? "Yeniden Baskı" : "Standart"}</p></div>
      </div>
    ),
    customer: (
      <div key="customer" className="grid grid-cols-2 gap-4 text-sm mb-4">
    ...
    products: (
      <table key="products" className="w-full text-sm border border-black/20 mb-4">
        <thead>
    ...
    adminNote: order.adminNote ? (
      <div key="adminNote" className="border border-black/20 rounded p-3 text-sm mb-4">
        <p className="text-black/50 text-xs mb-1">İç Not</p>
        <p>{order.adminNote as string}</p>
      </div>
    ) : null,
    processSteps: (
      <div key="processSteps" className="border-t-2 border-black pt-3 mb-4">
    ...
    trackingCode: (
      <div key="trackingCode" className="text-sm mb-2">
      ...
      <div id="makbuz" className="bg-white text-black border border-black/20 rounded-lg p-6 print:border-0 print:rounded-none">
        ...
        <div className="flex items-start justify-between border-b-2 border-black pb-3 mb-4">
          <div>
            {config.logoUrl ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={config.logoUrl} alt="Logo" className="h-10 mb-1 object-contain" />
            ) : (
              <p className="text-2xl font-bold">AnıBaskı</p>
            )}
            <p className="text-sm uppercase tracking-wide text-black/60">{config.headerTitle}</p>
        ...
        {/* Bölümler — config sırasına ve aç/kapa durumuna göre */}
        {config.sections.filter((s) => s.enabled).map((s) => sectionNodes[s.key])}

        {config.footerText && (
          <p className="text-xs text-black/50 border-t border-black/20 pt-2 mt-2">{config.footerText}</p>
        )}
      </div>
    </div>
  );
