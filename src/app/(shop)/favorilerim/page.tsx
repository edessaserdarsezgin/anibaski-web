...
export default async function FavorilerimPage() {
  ...
  const { data: favorites } = await adminDb
    .from("favorites")
    .select("productId, product:products(id, name, slug, description, basePrice, images, category:categories(name, slug))")
    .eq("userId", user.id)
  ...
  return (
    <div className="max-w-6xl mx-auto px-8 py-12">
      <div className="mb-8">
        <h1 className="font-serif text-4xl text-text">Favorilerim</h1>
      ...
      {products.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-32 gap-5">
          ...
          <a href="/urunler" className="px-6 py-2.5 border border-border rounded-full text-sm font-semibold text-text-light hover:border-primary hover:text-primary transition-colors">
            Ürünlere Göz At
          </a>
        </div>
      ) : (
